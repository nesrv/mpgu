...
У Gunicorn восемь воркеров — каждый держит соединения к БД. У Postgres лимит `max_connections` (часто 100). Плюс миграции, бэкапы, админ — соединения заканчиваются, приложение падает с «too many connections».

**PgBouncer** (или встроенный пул в облачных БД) — посредник: приложение открывает много коротких сессий к пулу, пул держит мало реальных соединений к Postgres. Режимы: session, transaction, statement — для Django обычно **transaction**.

`conn_max_age` в Django — пул на стороне приложения; вместе с PgBouncer нужно согласовывать (часто `conn_max_age=0` за балансировщиком, чтобы не держать лишние сокеты).

---

## Слайд 43: Кэш Redis + Django

Повторяющиеся тяжёлые запросы (каталог, справочники) — кандидаты в кэш. **django-redis** или встроенный backend на Redis: `cache.set('key', data, timeout=300)`.

Что кэшировать: фрагменты шаблонов (`{% cache %}`), результаты дорогих ORM-запросов, JSON для API. Что не кэшировать без стратегии инвалидации: персональные данные, корзина.

Инвалидация при обновлении модели — сигналы или явный `cache.delete_pattern` по префиксу. Иначе пользователи видят «вчерашний» каталог.

---

## Слайд 44: Фоновые задачи — Celery / RQ

Запрос не должен ждать отправки письма, ресайза картинки, отчёта на 10 секунд. **Очередь** (Redis/RabbitMQ) + **воркеры** (Celery, RQ) — задача уходит в фон, HTTP отвечает сразу.

Плюсы: стабильная latency для пользователя, повтор при сбое, rate limit на воркерах. Минусы: ещё один сервис в Docker Compose, мониторинг очереди (длина, failed jobs).

Связка с нагрузкой: пики трафика не убивают БД синхронными тяжёлыми операциями — они «растекаются» по времени в воркерах.

---

## Слайд 45: Django 6 — встроенный Tasks API (очередь в ядре фреймворка)

**С Django 6.0** в ядре появился контракт для фоновых задач: декоратор **`@task`**, постановка в очередь **`task.enqueue()`**, отслеживание **`TaskResult`** (статусы READY / RUNNING / SUCCESSFUL / FAILED), опции **`priority`** (−100…100), **`queue_name`**, отложенный запуск **`run_after`**.

Важно: Django задаёт **API и хранение описания задачи**, но **не поставляет продакшен-воркер**. В комплекте только:

- **`ImmediateBackend`** (по умолчанию) — выполняет задачу **сразу в том же потоке** (как обычный вызов функции — для HTTP это не «фон»).
- **`DummyBackend`** — кладёт задачу в очередь без исполнения (тесты, инспекция).

Для реального фона подключают **внешние бэкенды**: например `django-tasks-database` (очередь в PostgreSQL), `django-tasks-celery`, `django-tasks-huey`, `django-tasks-threads` (пул потоков — осторожно на CPU-bound).

Аргументы в `enqueue()` должны быть **JSON-сериализуемы** (после round-trip через JSON).
...
def view(request):
    result = send_welcome_email.enqueue(request.user.pk)  # TaskResult
    # result.id — потом get_result() на бэкендах с supports_get_result
```

Итог: **единый способ описать задачу в проекте** независимо от того, Celery под капотом или БД-очередь; продакшен по-прежнему требует выбрать и настроить бэкенд.

---

## Слайд 46: Rate limiting в Nginx

Простая защита API от перегруза и ботов: **limit_req_zone** + **limit_req** в `location`. Например зона по `$binary_remote_addr`, 10 r/s с burst.

Не путать с полноценной аутентификацией — это грубый предохранитель. Для публичного API — разумный default; для админки — отдельный location со stricter лимитом.

---

## Слайд 47: Ошибки в production — Sentry

Prometheus показывает *что* тормозит и сколько 500-х. **Sentry** (или аналог) — *где* упало: stack trace, release, пользователь, breadcrumbs. Интеграция одной строкой в Django settings.

После деплоя первые часы критичны: новый код → новые исключения. Алерт в Slack при росте error rate — быстрее, чем ждать звонков от пользователей.

---

## Слайд 48: Логи, метрики, трейсы

Три столпа наблюдаемости: **логи** (структурированный JSON, correlation id), **метрики** (уже Prometheus), **трейсы** (OpenTelemetry — кто вызвал кого, latency по спанам).

На одном VPS полный стек трейсинга редко нужен; но понимание цепочки «запрос → Django → БД → Redis» помогает при инцидентах. Минимум в проде: логи + метрики + Sentry.

---

## Слайд 49: Мини-чек-лист после деплоя + развитие темы

1. `curl` health / smoke страница  
2. Один короткий **wrk** или ab — нет ли резкого просадки к baseline  
3. Grafana: нет ли скачка ошибок и latency  
4. Запас по соединениям к БД и место на диске  

**Куда развиваться дальше:** read replicas для чтения, CDN для статики, blue-green деплой, chaos-инженерия на staging. Тема одна — **управляемая нагрузка**; инструменты меняются, вопросы остаются: где узкое место, как не будить вас ночью, как откатиться за пять минут.
