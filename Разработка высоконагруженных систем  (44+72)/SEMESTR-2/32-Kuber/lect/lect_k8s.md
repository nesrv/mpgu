...
* **Практика:** minikube / kind / k3d / k3s, свои YAML, потом Helm или Kustomize — **руками один раз** пройти бесценно

---

# Слайд 41. «Kubernetes» — не один установщик

* Слово **Kubernetes (K8s)** — это **стандарт и API**: Pod, Deployment, Service, `kubectl`, YAML — **один язык** для всех совместимых кластеров
* На практике вы ставите **конкретную сборку** (дистрибутив): **kubeadm**, **облако** (EKS, GKE, AKS), **OpenShift**, **RKE**, **K3s** и т.д. — как **разные дистрибутивы Linux**: ядро одно, **упаковка** разная
* Для разработчика важно: **манифесты и команды** в основном **те же**; отличаются **установка, дефолты, сеть, размер** кластера

---

# Слайд 42. Что такое K3s

* **K3s** — **лёгкий дистрибутив Kubernetes** (проект Rancher / SUSE), **не другой оркестратор**
* Цель: **меньше ресурсов**, **проще старт** на одной машине, **edge**, слабый VPS, учебные стенды
* **Совместим с Kubernetes API** (сертифицированная сборка): те же объекты, тот же `kubectl`
* Внутри могут быть **другие дефолты** (встроенные компоненты, хранилище состояния, CNI) — но **модель** та же

---

# Слайд 43. K8s «полный» и K3s — на пальцах

| | **Типичный «большой» K8s** | **K3s** |
|---|---------------------------|---------|
| **Ресурсы** | Обычно **больше** RAM/CPU на control plane | **Меньше**, проще на маленьком железе |
| **Установка** | Часто **много шагов** или managed в облаке | Часто **одна команда**, быстрый старт |
| **Где любят** | Прод, много нод, команды платформы | Edge, IoT, малые кластеры, «быстро поднять» |
| **API и YAML** | Стандарт | **Тот же стандарт** |

* Аналогия: **полноразмерный автобус** vs **компактный фургон** — **правила дороги** (API) **одинаковые**

---

# Слайд 44. Не путать: K3s, k3d, minikube, kind

* **K3s** — **реальный лёгкий Kubernetes** на машине (или нескольких нодах)
* **k3d** — **K3s внутри Docker** (удобно локально: «кластер в контейнерах»)
* **minikube** — локальный кластер (часто одна нода), много документации для учебы
* **kind** — кластер **в Docker** (часто для CI и быстрых экспериментов)

**Итог:** это **способы получить Kubernetes на столе**, а не «K3s вместо K8s как другая платформа»

---

# Слайд 45. Когда что иметь в виду

* **Учёба / первая лаба на ноутбуке:** чаще **minikube** или **kind** / **k3d** — по заданию преподавателя
* **Один слабый VPS, мало памяти:** смотрят **K3s** или **k3d**
* **Прод в компании / облако:** обычно **полный K8s** или **managed** (EKS, GKE, AKS) — K3s там тоже бывает, но это **выбор платформы**, не экзамен по «правильному K8s»

---

# Слайд 46. Почему k3d — 🔥 для практических занятий

**Сравнение вариантов для учебной аудитории:**

| Критерий | Docker Desktop K8s | k3s | k3d |
|----------|-------------------|-----|-----|
| Платформа | Win / macOS / Linux | Linux | Win / macOS / Linux |
| Сложность | очень низкая | средняя | низкая |
| Похожесть на прод | ⭐⭐ | ⭐⭐⭐⭐⭐ | ⭐⭐⭐⭐ |
| Работа с образами | локально | через registry | локально |
| Мульти-нода | ❌ | ⚠️ сложно | ✅ |
| Скорость старта | средняя | медленная | быстрая |

📌 **Идеальный мост:** Docker Desktop → **k3d** → **k3s** → **managed Kubernetes**

*На занятиях k3d часто удобен: **один Docker**, **быстрый** подъём кластера, **локальные образы** без публикации в registry, возможность **нескольких нод** для демонстрации.*

---

# Слайд 47. Kubernetes как контрольный автомат
