...
Знание CI/CD — это мастхэв для любого современного разработчика и абсолютно критичный навык для DevOps-инженера. Компании готовы платить больше тем, кто умеет настраивать пайплайны.
...
```
Что если сервер сам будет:
- Проверять код при каждом коммите?
- Запускать тесты?
- Деплоить на тестовый сервер?
...
Через месяц уставшая команда допускает ошибку — деплоят не ту ветку. Половина функционала исчезает с сайта. Откат занимает 3 часа. А бизнес в это время потерял большие деньги.
...
И тут на сцену выходит этот самый CI/CD, но что скрыто за этими 4 буквами? — разберём в следующем разделе.
...
```
На предыдущих уроках мы увидели проблемы ручных процессов и поняли, что нужна автоматизация.
 Сейчас мы разберём первую и самую важную часть этой автоматизации — Continuous Integration или — непрерывную интеграцию. Это фундамент, на котором строится весь современный процесс разработки.
