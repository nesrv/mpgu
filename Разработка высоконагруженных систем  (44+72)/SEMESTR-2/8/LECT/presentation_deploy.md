...
Платформа, где **инфраструктура скрыта** от разработчика. Вы управляете только приложением — среда выполнения, сеть, ОС, обновления — забота провайдера.

### Примеры платформ

- **Railway** — простота, Python из коробки
- **Render** — free tier, автоматические деплои
- **Fly.io** — edge-расположение, низкая латентность
- **Heroku** — классика (платный после 2022)

### Модель работы

```
git push → автоматическая сборка → запуск → готовый URL
```
