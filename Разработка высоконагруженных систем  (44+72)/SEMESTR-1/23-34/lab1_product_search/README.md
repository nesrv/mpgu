...
```bash
mkdir lab1_product_search
cd lab1_product_search
```
...
**1.3. Создать виртуальное окружение Python**
```bash
python -m venv venv
...
# Подключение к OpenSearch без SSL (для разработки)
client = OpenSearch(
...
# Индексация товара в OpenSearch
def index_product(product):
    ...
    # Сохраняем документ с ID товара
...
def search_products(query, category=None, min_price=None, max_price=None):
    must = []  # Условия поиска (должны совпадать)
    filters = []  # Фильтры (точное совпадение)
    ...
    if query:
        must.append({
            "multi_match": {
                "query": query,
                "fields": ["name^3", "description"],  # name^3 - утроенный вес
                "type": "best_fields"  # Лучшее совпадение среди полей
            }
        })
    ...
    if min_price or max_price:
        ...
        if min_price:
            range_filter["range"]["price"]["gte"] = min_price  # greater than or equal
        if max_price:
            range_filter["range"]["price"]["lte"] = max_price  # less than or equal
        filters.append(range_filter)
    
    # Формирование запроса
    body = {
        "query": {
            "bool": {  # Булев запрос для комбинации условий
                "must": must if must else [{"match_all": {}}],  # Если нет запроса - все товары
                "filter": filters  # Применяем фильтры
            }
        },
        "aggs": {  # Агрегации для фасетного поиска
            "categories": {"terms": {"field": "category"}},  # Подсчет по категориям
            "price_ranges": {  # Подсчет по ценовым диапазонам
                "range": {
                    "field": "price",
                    "ranges": [
                        {"to": 1000},  # До 1000
...
def suggest_products(prefix):
    body = {
        "suggest": {
            "product-suggest": {
                "prefix": prefix,  # Начало слова
                "completion": {"field": "name.suggest"}  # Поле для автодополнения
            }
        }
    }
    return client.search(index=INDEX_NAME, body=body)
...
{
  "name": "iPad Air",
  "description": "Планшет Apple с дисплеем 10.9 дюймов",
  "price": 59990,
  "category": "Планшеты",
  "popularity": 75
}
...
### Поиск по запросу
GET http://localhost:8000/search?q=iPhone

### Поиск с фильтрами
GET http://localhost:8000/search?q=смартфон&category=Смартфоны&min_price=50000&max_price=100000

### Автодополнение
GET http://localhost:8000/suggest?q=iph

### Поиск по категории
GET http://localhost:8000/search?category=Наушники

### Поиск по цене
GET http://localhost:8000/search?min_price=10000&max_price=30000
```
