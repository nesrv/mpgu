...
class Query:
    ...
    async def messages(self) -> list[MessageType]:
        ...
        """
        from message_resolvers import get_all_messages
    ...
    async def message(self, id: int) -> MessageType | None:
        ...
        """
        from message_resolvers import get_message_by_id
...
class Mutation:
    ...
    async def create_message(
        ...
        - metadata: JSON | None - дополнительные данные в формате JSON (опционально)
        - stats: JSON | None - статистика в формате JSON (опционально)
        ...
        """
        from message_resolvers import create_message
        metadata_dict = metadata if isinstance(metadata, dict) else None
        stats_dict = stats if isinstance(stats, dict) else None
        return await create_message(author_id, content, title, metadata_dict, stats_dict)
    ...
    async def update_message(
        ...
        - metadata: JSON | None - новые дополнительные данные (опционально)
        - stats: JSON | None - новая статистика (опционально)
        ...
        """
        from message_resolvers import update_message
        metadata_dict = metadata if isinstance(metadata, dict) else None
        stats_dict = stats if isinstance(stats, dict) else None
        return await update_message(message_id, title, content, metadata_dict, stats_dict)
    ...
    async def delete_message(self, message_id: int) -> bool:
        ...
        Возвращает: true если сообщение удалено, false если не найдено
        """
        from message_resolvers import delete_message
