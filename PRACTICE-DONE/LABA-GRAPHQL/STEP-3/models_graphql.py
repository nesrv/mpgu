...
class Query:
    ...
    async def users(self) -> list[UserType]:
        ...
        Выполняет SQL запрос к БД и возвращает список всех пользователей.
        ...
        Примечание:
        - Использует асинхронную сессию SQLAlchemy для работы с БД
        - RowMapping автоматически преобразуется в словарь для создания UserType
        """
        async with AsyncSessionLocal() as session:
            result = await session.execute(
    ...
    async def user(self, id: int) -> UserType | None:
        ...
        Выполняет SQL запрос к БД для поиска пользователя с указанным ID.
